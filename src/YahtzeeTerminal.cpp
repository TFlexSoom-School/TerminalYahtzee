...
{
	/* Construct Player "Array" */
	Player *players = new Player[numPlayers];

	/* Get Set of 5 Dice */
	Dice *dice = new Dice();

	/* Have index of current Player */
	int player_index;

	/* Construct UI providing pointers to important variables */
	UI *ui = new UI(players, &numPlayers, dice, &player_index);
	...
	do
	{
		...
		do
		{
			if (playerStillPlaying(&players[player_index]))
			{
				do
				{
					dice->rollDice();
				} while (!scoreOrReroll(&players[player_index], dice, ui));
			}
			dice->unHoldAllDice();
			player_index += (player_index >= numPlayers - 1 ? -1 * player_index : 1);
...
int ofKind(Dice *dice, int arg)
{
	...
	for (int i = 1; i < 7; i++)
	{
		if (containsSet < dice->getResult(i))
			containsSet = dice->getResult(i);
	}
	if (containsSet >= arg)
		return dice->getSum();
	return 0;
