...
void UI::start()
{
   this->wnd = newwin(HEIGHT, WIDTH, START, START);
...
void UI::updatePrint()
{
   ...
   for (int i = 0; i < 5; i++)
   {
      this->printDice(this->dice_array->getDice(i), i + 1); /* Print 5 Dice */
   }
...
void UI::printChart()
{
   ...
   mvwprintw(this->wnd, 21, 1, "|     1s     |   %s    | 3 of a Kind  |   %s    |", this->playerValue(0).c_str(), this->playerValue(7).c_str());
   mvwprintw(this->wnd, 22, 1, "|     2s     |   %s    | 4 of a Kind  |   %s    |", this->playerValue(1).c_str(), this->playerValue(8).c_str());
   mvwprintw(this->wnd, 23, 1, "|     3s     |   %s    | Full  House  |   %s    |", this->playerValue(2).c_str(), this->playerValue(9).c_str());
   mvwprintw(this->wnd, 24, 1, "|     4s     |   %s    |Small Straight|   %s    |", this->playerValue(3).c_str(), this->playerValue(10).c_str());
   mvwprintw(this->wnd, 25, 1, "|     5s     |   %s    |Large Straight|   %s    |", this->playerValue(4).c_str(), this->playerValue(11).c_str());
...
int UI::getUserInput()
{
   ...
   do
   {
      ...
      this->updatePrint(); /* All Inputs Except Select Refresh */
   } while (true);
}
...
void UI::move(int dir)
{
   ...
   case 0: /* Up */
      ...
      break;
   case 1: /* Right */
      /* Can only go farther than 1 if highlighting dice */
      ...
      break;
   case 2: /* Down */
      /* Clamp Selector col to 0 or 1 depending on row */
      ...
      break;
   case 3: /* Left */
      /* As long as it is not clamped already (row 6 col = 1) */
